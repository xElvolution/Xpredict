...
export function assertCreateAgent(input) {
    ...
    if (!input.name?.trim())
        throw new XPredictValidationError('name is required');
    if (!AGENT_STYLES.includes(input.style)) {
        throw new XPredictValidationError(`style must be one of: ${AGENT_STYLES.join(', ')}`);
    }
    if (!input.focus?.length)
        throw new XPredictValidationError('focus must include at least one category');
    for (const c of input.focus) {
        if (!CATEGORIES.includes(c))
            throw new XPredictValidationError(`invalid category: ${c}`);
    }
}
export function assertProposeMarket(input) {
    ...
    if (!input.subtitle?.trim())
        throw new XPredictValidationError('subtitle is required');
